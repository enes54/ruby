...
def chiffre()
	...
	puts "il ya #{r} pseudo qui contient un numero"
end
...
def lettre()
	...
	puts "il y a #{b} pseudo qui contiennent aude."
end
...
def majuscule()
	...
	@a.times do |i|
		nom = @journalist[i]
		first_letter = nom[1]
		nom_grand = nom[1].upcase
		if first_letter == nom_grand
			c +=1
		end
	end	
	puts "il y a #{c} pseudo qui commence par une majuscule" 
end
...
def majuscule2()
	...
	@a.times do |i|
		nom = @journalist[i] 
		downcase = nom.downcase
		if nom != downcase
			e +=1
		end
	end
	puts "il ya #{e} pseudo qui contiennent une majuscule"
end

def tiré
	e = @journalist.join
	underscore = e.scan(/_/).count
	puts "il y a #{underscore} '_' dans tous les pseudo confondus"
end
